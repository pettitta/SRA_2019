...
```{r}
fpp <- c("i","me","my","mine","we","us","our","ours","im","myself","ourselves")
...
```{r}
daily.imeans <- plyr::ddply(daily_text_pronouns, "participant_id", summarize,imean.fpp=mean(fpp_proportion, na.rm=TRUE))

daily.imeans$imean.fpp.c <- scale(daily.imeans$imean.fpp,center=TRUE,scale=FALSE)

daily_text_pronouns <- merge(daily_text_pronouns,daily.imeans,by="participant_id")

daily_text_pronouns$fpp.state <- daily_text_pronouns$fpp - daily_text_pronouns$imean.fpp

```

```{r}
lg.fit.q4 <- lme(fixed= daily.state ~ fpp.state,
                 random= ~ 1  |participant_id, 
                 correlation = corAR1(),
...
model_3_1 <- lmer(daily ~ fpp.state*n.state*fpp.state + (1|participant_id), daily_text_pronouns)

summary(model_3_1)

plot(daily_text_pronouns$daily,daily_text_pronouns$imean.fpp)
```
