...
rm(list=ls())
packages <- c("rio","tidyverse","reshape","lme4","interactions","jtools","lmerTest","Amelia","mice","lavaan","semTools","janitor","stargazer","plotluck","splitstackshape","gratia","mgcv","Amelia","lubridate","here","fuzzyjoin","Metrics","readxl","tidytext","sjPlot","sjmisc")
...
hlm_data_sentiment <- merge(daily_text_sentiment, daily_ema_data, by=c("participant_id","date")) %>%
  select(participant_id,date,daily_sentiment,n,site,daily,day_of_week,weekend,social,dating,timestamp_ema=timestamp_est)
...
hlm_data_sentiment_test <- hlm_data_sentiment %>%
  filter(participant_id=="2018")

text_tidy_data_filtered_test <- text_tidy_data_filtered %>%
  filter(participant_id=="2018")
library(data.table)
...
daily_text_pronouns$fpp_n.state.s <- scale(daily_text_pronouns$fpp_n.state)

daily_text_pronouns$imean.fpp_n.c.s <- scale(daily_text_pronouns$imean.fpp_n.c)
...
model_3_1 <- lmer(daily.state ~ fpp_n.state.s*imean.fpp_n.c.s + (1|participant_id), daily_text_pronouns,REML=FALSE)

summary(model_3_1)

plot(daily_text_pronouns$daily,daily_text_pronouns$fpp_n)

interact_plot(model_3_1,imean.fpp_n.c.s,fpp_n.state.s,interval=TRUE,plot.points=FALSE)

```

```{r}
packages <- c("rio","tidyverse","reshape","lme4","interactions","jtools","janitor","merTools","plotly")
if (length(setdiff(packages, rownames(installed.packages()))) > 0) {
  install.packages(setdiff(packages, rownames(installed.packages())))  
}
lapply(packages, library, character.only = TRUE)
...
# Simple function for plotting the random effects coefficients form an lmer() output
REplot <- function(out) {
...
teplot_model(model_3_1, type = "re", facet.grid=FALSE)
