...
as_datetime(as.numeric(daily_ema_data[["time_completed"]])/1000, origin="1970-01-01",tz="America/New_York")

as.POSIXct(daily_ema_data$time_completed, format="%Y-%m-%d %H:%M:%S",tz="America/New_York")

daily_ema_data$timestamp_est <- as.POSIXct(daily_ema_data$time_completed, format="%Y-%m-%d %H:%M:%S",tz="America/New_York")
...
socialsjstats::std_beta(base_model)
...
ggplot(test, aes(x = fpp_zscore, y = daily_zscore,color=participant_id)) +
  geom_point() +
  stat_smooth(method = "lm", col = "red")
